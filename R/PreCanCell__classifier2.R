...
#'
#' @description \code{PreCanCell_classifier2} Identify malignant and non-malignant cells across cancer types.
#'
...
#' results <- PreCanCell_classifier2(testdata, 2)


PreCanCell_classifier2 <- function(testdata, cores) {
  ...
  res <- suppressWarnings(
    foreach::foreach(i = seq_len(length(ModelList)), .combine = "cbind", .packages = "caret", .export = c("ModelList","testdata"), .errorhandling = "remove") %dopar% {
    pred <- caret::predict(ModelList[[i]], newdata = testdata)
    })
